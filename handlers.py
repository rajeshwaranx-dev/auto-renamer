...
async def init_pyro_client(api_id, api_hash, session_string="", bot_token=""):
    ...
    try:
        if session_string:
            ...
            _pyro_client = PyroClient("leech_bot_pyro", api_id=api_id, api_hash=api_hash, bot_token=bot_token, workdir="/root/leechbot")
        else:
...
async def _pyro_download(chat_id, message_id, dest_path):
...
async def handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in matched:
        ...
        try:
            ...
            caption = _build_caption(caption_tpl, filename=original_name, newname=new_name, name=name_no_ext, ext=ext, prefix=prefix, size=human_size(file_size))
            log.info("Uploading | user=%s | %s", user["name"], new_name)
            await _pyro_upload(pyro=pyro, message=message, dest_channel=dest, file_path=processed_path, new_name=new_name, caption=caption, thumb_path=thumb_dl_path)
...
async def handle_thumb_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message.reply_text("Thumbnail saved!", parse_mode=ParseMode.HTML)
